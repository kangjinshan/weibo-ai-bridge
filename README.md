...
- **流式回复**: 收到用户消息后会先发送“正在处理中”提示，真实回复会在同一条流式消息内继续追加
...
- **处理中提示合并发送**: “已收到消息，正在处理中，请稍候。” 不再单独占一条消息，而是作为同一轮流式回复的前缀，真实正文会从下一行接续输出。
...
- `/btw [content]` - 向当前正在进行的交互式会话插入一条补充消息
- `/model` - 显示当前使用的模型
- `/dir` - 显示当前工作目录
- `/status` - 显示当前会话状态
