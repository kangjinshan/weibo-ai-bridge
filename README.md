...
这个项目适合把一台本地机器上的编码 Agent、审批确认、项目会话和微博私信入口连在一起。内置的 `weibo-skill-api` 还可以让 Agent 复用同一套微博凭证，查询热搜/智搜、检查微博状态、参与超话互动、上传媒体、管理定时任务，并生成创作者数据、激励计划和互动内容分析。
