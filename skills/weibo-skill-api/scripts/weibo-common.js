 ...
 *
 * 提供以下公共能力：
 * - 复用 weibo-ai-bridge 的配置与 token 缓存
 * - HTTP 请求工具
 * - 日志工具
 */
...
const TOKEN_HELPER_PATH = path.join(SCRIPT_DIR, 'ensure_token.sh');
...
export const COMMON_ERROR_MESSAGES = {
  40100: 'Token 无效或已过期，请检查 bridge 配置后重试',
...
/**
 * 加载配置
 * bridge 集成版不维护独立凭证文件，调用方应通过 ensure_token.sh 读取配置。
...
/**
 * bridge 集成版不保存独立凭证，保留此函数仅用于兼容旧导入。
 * @param {object} config - 配置对象
 */
export async function saveLocalConfig(config) {
  throw new ConfigError(
    'weibo-ai-bridge 集成版不写入独立凭证文件；请在 bridge 的 config/config.toml、/etc/weibo-ai-bridge/config.toml 或环境变量中配置微博凭证'
  );
}
...
export async function handleLoginCommand(appId, appSecret, skillName = '微博 Skill') {
  throw new ConfigError(
    'weibo-ai-bridge 集成版无需执行 login，也不会保存 ~/.weibo-skill/config.json。\n' +
    '请将 App ID / App Secret 配置到 bridge 的 config/config.toml、/etc/weibo-ai-bridge/config.toml，或设置 WEIBO_APP_ID / WEIBO_APP_SECRET。\n' +
    '业务命令会通过 scripts/ensure_token.sh 自动读取或刷新 token。'
  );
}
...
export async function getValidTokenForCommand() {
  try {
    const { stdout } = await execFileAsync('bash', [TOKEN_HELPER_PATH, '--print-token'], {
      env: process.env,
      maxBuffer: 1024 * 1024,
    });
    const token = stdout.trim();
    if (!token) {
      throw new TokenError('ensure_token.sh 未返回 token');
    }
    return token;
  } catch (err) {
    ...
    const message = stderr || err.message || '获取 token 失败';
    throw new ConfigError(
      `无法从 weibo-ai-bridge 配置获取 token：${message}\n` +
      '请确认 CONFIG_PATH、/etc/weibo-ai-bridge/config.toml、仓库 config/config.toml 或 WEIBO_APP_ID / WEIBO_APP_SECRET 可用。'
    );
  }
}
