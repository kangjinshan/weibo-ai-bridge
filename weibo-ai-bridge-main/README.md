...
- **上下文记忆**: 支持永久保留对话历史，可创建/切换/恢复会话
- **消息路由**: 智能路由消息到对应的 AI Agent
- **命令处理**: 支持创建会话、切换会话、切换 Agent 等丰富命令
...
- `/new [agent]` - 创建新会话（如：`/new` 或 `/new codex`）
- `/resume [session-id]` - 列出所有会话或切换到指定会话
...
- **App ID**: 应用唯一标识，例如：`5288006887407627`
...
**WebSocket 连接 URL**：
```
ws://open-im.api.weibo.com/ws/stream?app_id=5288006887407627&token=your-64-char-token
```
