...
func TestResolveTextDelta(t *testing.T) {
	tests := []struct {
		name         string
		previous     string
		next         string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			delta, snapshot := resolveTextDelta(tt.previous, tt.next)
			if delta != tt.wantDelta {
				t.Errorf("resolveTextDelta(%q, %q) delta = %q, want %q", tt.previous, tt.next, delta, tt.wantDelta)
			}
			if snapshot != tt.wantSnapshot {
				t.Errorf("resolveTextDelta(%q, %q) snapshot = %q, want %q", tt.previous, tt.next, snapshot, tt.wantSnapshot)
			}
			// Verify the delta is valid UTF-8
			for _, r := range delta {
				if r == 0xFFFD {
					t.Errorf("resolveTextDelta(%q, %q) delta contains invalid UTF-8 replacement character", tt.previous, tt.next)
				}
			}
		})
	}
