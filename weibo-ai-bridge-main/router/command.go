...
func (h *CommandHandler) Handle(msg *Message) (*Response, error) {
	...
	case "/resume":
		return h.handleResume(msg.UserID, args)
	case "/switch":
...
func (h *CommandHandler) handleNew(userID string, args []string) (*Response, error) {
	...
	// 创建新会话
	newSession := h.sessionManager.Create("", userID, agentType)
...
// handleResume 处理恢复会话命令
func (h *CommandHandler) handleResume(userID string, args []string) (*Response, error) {
	// 如果没有参数，列出用户的所有会话
	if len(args) == 0 {
		...
		for i, sess := range sessions {
			listText += fmt.Sprintf("%d. %s (Agent: %s, State: %s, Updated: %s)\n",
				i+1, sess.ID, sess.AgentType, sess.State,
				sess.UpdatedAt.Format("2006-01-02 15:04:05"))
		}

		return &Response{
			Success: true,
			Content: fmt.Sprintf("Your sessions:\n%s\nTo resume a session, use /resume <session_id>", listText),
		}, nil
	}
	...
	if sess.UserID != userID {
		return &Response{
			Success: false,
			Content: "You don't have permission to access this session.",
		}, nil
	}
	...
	return &Response{
		Success: true,
		Content: fmt.Sprintf("Resumed session: %s (Agent: %s)", sess.ID, sess.AgentType),
	}, nil
}
