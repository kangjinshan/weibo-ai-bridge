...
func TestCommandHandler_Handle_Help(t *testing.T) {
	...
	assert.True(t, strings.HasPrefix(resp.Content, "## 可用命令"), "help should be Markdown with a heading")
	assert.Contains(t, resp.Content, "- `/help`")
	assert.Contains(t, resp.Content, "### 会话")
	assert.Contains(t, resp.Content, "### 工具")
	assert.Contains(t, resp.Content, "/help")
